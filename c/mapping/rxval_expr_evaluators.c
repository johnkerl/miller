...
// ----------------------------------------------------------------
rxval_evaluator_t* rxval_evaluator_alloc_from_astx(mlr_dsl_ast_node_t* pnode, fmgr_t* pfmgr,
	...
	for (sllve_t* pe = pnode->pchildren->phead; pe != NULL; pe = pe->pnext) {
		...
		*ppair = (map_literal_pair_evaluator_t) {
		...
		if (pright->type == MD_AST_NODE_TYPE_MAP_LITERAL_VALUE) {
			ppair->pxval_evaluator = rxval_evaluator_alloc_from_astx(pvalnode, pfmgr, type_inferencing, context_flags);
