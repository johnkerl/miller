...
static void mapper_rename_usage(FILE* o, char* argv0, char* verb) {
	...
	fprintf(o, "--gsub           Do global replacement within each field name rather than\n");
	fprintf(o, "                 first-match replacement.\n");
	fprintf(o, "Examples:\n");
	fprintf(o, "%s %s 'Date_[0-9]+,Date,'  Rename all such fields to be \"Date\"\n", argv0, verb);
	fprintf(o, "%s %s '\"Date_[0-9]+\",Date' Same\n", argv0, verb);
	fprintf(o, "%s %s 'Date_([0-9]+).*,\\1' Rename all such fields to be of the form 20151015\n", argv0, verb);
	fprintf(o, "%s %s '\"name\"i,Name'       Rename \"name\", \"Name\", \"NAME\", etc. to \"Name\"\n", argv0, verb);
}
