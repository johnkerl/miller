	...
	int type_inferencing, int context_flags)
{
	...
	for (sllve_t* pe = pnode->pchildren->phead; pe != NULL; pe = pe->pnext) {
		...
		if (pchild->type == MD_AST_NODE_TYPE_ALL || pchild->type == MD_AST_NODE_TYPE_FULL_OOSVAR) {
			...
			punset_item->poosvar_keylist_evaluators = allocate_keylist_evaluators_from_oosvar_node(
				pcst, pchild, type_inferencing, context_flags);
			sllv_append(pstate->punset_items, punset_item);

		} else if (pchild->type == MD_AST_NODE_TYPE_LOCAL_NON_MAP_VARIABLE) {
	...
	cst_outputs_t* pcst_outputs)
{
	...
	local_stack_frame_assign_non_map(pframe, punset_item->local_variable_frame_relative_index, mv_absent());
}
