	...
	if (streq(popts->ifile_fmt, "gen")) {
		...
		if (popts->use_mmap_for_read)
			return lrec_reader_mmap_csvlite_alloc(popts->irs, popts->ifs, popts->allow_repeat_ifs,
				popts->use_implicit_csv_header, popts->comment_string);
		else
			return lrec_reader_stdio_csvlite_alloc(popts->irs, popts->ifs, popts->allow_repeat_ifs,
				popts->use_implicit_csv_header, popts->comment_string);
	} else if (streq(popts->ifile_fmt, "nidx")) {
		if (popts->use_mmap_for_read)
			return lrec_reader_mmap_nidx_alloc(popts->irs, popts->ifs, popts->allow_repeat_ifs, popts->comment_string);
		else
			return lrec_reader_stdio_nidx_alloc(popts->irs, popts->ifs, popts->allow_repeat_ifs, popts->comment_string);
	} else if (streq(popts->ifile_fmt, "xtab")) {
		if (popts->use_mmap_for_read)
			return lrec_reader_mmap_xtab_alloc(popts->ifs, popts->ips, popts->allow_repeat_ips, popts->comment_string);
		else
			return lrec_reader_stdio_xtab_alloc(popts->ifs, popts->ips, popts->allow_repeat_ips, popts->comment_string);
