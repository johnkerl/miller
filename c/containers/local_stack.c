...
local_stack_t* local_stack_alloc() {
	...
	pstack->ppframes = mlr_malloc_or_die(pstack->num_allocated * sizeof(local_stack_frame_t*));
	memset(pstack->ppframes, 0, pstack->num_allocated * sizeof(local_stack_frame_t*));

	pstack->pbase_frame = local_stack_frame_alloc_unfenced();
	local_stack_push(pstack, pstack->pbase_frame);
