...
static char* test_lrec_dkvp_api() {
	...
	lrec_t* prec = lrec_parse_dkvp(line, ',', '=', FALSE);
...
static char* test_lrec_nidx_api() {
	...
	lrec_t* prec = lrec_parse_nidx(line, ',', FALSE);
	mu_assert_lf(prec->field_count == 4);
...
static char* test_lrec_csv_api() {
	...
	slls_t* hdr_fields = split_csv_header_line(hdr_line, ',', FALSE);
	hdr_keeper_t* phdr_keeper = hdr_keeper_alloc(hdr_line, hdr_fields);

	char* data_line_1 = strdup("2,3,4,5");
	lrec_t* prec_1 = lrec_parse_csv(phdr_keeper, data_line_1, ',', FALSE);

	char* data_line_2 = strdup("6,7,8,9");
	lrec_t* prec_2 = lrec_parse_csv(phdr_keeper, data_line_2, ',', FALSE);
...
static char* test_lrec_xtab_api() {
	...
	lrec_t* prec = lrec_parse_xtab(pxtab_lines, ' ', TRUE);
	mu_assert_lf(prec->field_count == 4);
