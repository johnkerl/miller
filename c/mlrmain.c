...
int main(int argc, char** argv) {
	...
	lrec_reader_t* plrec_reader_stdio = popts->plrec_reader_stdio;
	lrec_reader_t* plrec_reader_mmap  = popts->plrec_reader_mmap;
	sllv_t*        pmapper_list       = popts->pmapper_list;
	lrec_writer_t* plrec_writer       = popts->plrec_writer;
	char**         filenames          = popts->filenames;

	int ok = do_stream_chained(filenames, popts->use_file_reader_mmap, plrec_reader_stdio, plrec_reader_mmap,
