...
* `--box`: Use Unicode box-drawing tabular format for input and output data.
...
* `--ibox`: Use Unicode box-drawing tabular format for input data.
...
* `--obox`: Use Unicode box-drawing tabular format for output data.
...
The letters `c`, `t`, `j`, `l`, `d`, `n`, `x`, `p`, `m`, `y`, and `u` refer to formats CSV, TSV, JSON, JSON Lines,
DKVP, NIDX, XTAB, PPRINT, markdown, YAML, and box-drawing tabular, respectively. DCF is also supported (use `--dcf` for DCF in and out). Note that markdown format is available for
output only.

| In \ out  | **CSV** | **TSV** | **JSON** | **JSONL** | **DKVP** | **NIDX** | **XTAB** | **PPRINT** | **Markdown** | **YAML** | **Box** |
|------------|---------|---------|----------|-----------|----------|----------|----------|------------|--------------|----------|---------|
| **CSV**    |         | `--c2t` | `--c2j`  | `--c2l`   | `--c2d`  | `--c2n`  | `--c2x`  | `--c2p`    | `--c2m`      | `--c2y`  | `--c2u` |
| **TSV**    | `--t2c` |         | `--t2j`  | `--t2l`   | `--t2d`  | `--t2n`  | `--t2x`  | `--t2p`    | `--t2m`      | `--t2y`  | `--t2u` |
| **JSON**   | `--j2c` | `--j2t` |          | `--j2l`   | `--j2d`  | `--j2n`  | `--j2x`  | `--j2p`    | `--j2m`      | `--j2y`  | `--j2u` |
| **JSONL**  | `--l2c` | `--l2t` | `--l2j`  |           | `--l2d`  | `--l2n`  | `--l2x`  | `--l2p`    | `--l2m`      | `--l2y`  | `--l2u` |
| **DKVP**   | `--d2c` | `--d2t` | `--d2j`  | `--d2l`   |          | `--d2n`  | `--d2x`  | `--d2p`    | `--d2m`      | `--d2y`  | `--d2u` |
| **NIDX**   | `--n2c` | `--n2t` | `--n2j`  | `--n2l`   | `--n2d`  |          | `--n2x`  | `--n2p`    | `--n2m`      | `--n2y`  | `--n2u` |
| **XTAB**   | `--x2c` | `--x2t` | `--x2j`  | `--x2l`   | `--x2d`  | `--x2n`  |          | `--x2p`    | `--x2m`      | `--x2y`  | `--x2u` |
| **PPRINT** | `--p2c` | `--p2t` | `--p2j`  | `--p2l`   | `--p2d`  | `--p2n`  | `--p2x`  |            | `--p2m`      | `--p2y`  | `--p2u` |
| **Markdown** | `--m2c` | `--m2t` | `--m2j`  | `--m2l`   | `--m2d`  | `--m2n`  | `--m2x`  | `--m2p`    |              | `--m2y`  | `--m2u` |
| **YAML**   | `--y2c` | `--y2t` | `--y2j`  | `--y2l`   | `--y2d`  | `--y2n`  | `--y2x`  | `--y2p`    | `--y2m`      | `--y2y`  | `--y2u` |
| **Box**    | `--u2c` | `--u2t` | `--u2j`  | `--u2l`   | `--u2d`  | `--u2n`  | `--u2x`  | `--u2p`    | `--u2m`      | `--u2y`  |         |
